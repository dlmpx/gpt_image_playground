                              ...
                              {editingNotesId === candidate.id ? (
                                <div className="mt-1 px-1" onClick={(e) => e.stopPropagation()}>
                                  <textarea
                                    ...
                                    onBlur={() => {
                                      updateCandidateNotes(candidate.id, editingNotesText)
                                    ...
                                    placeholder="备注..."
                                    rows={2}
                                    className="w-full px-1.5 py-1 rounded border border-purple-200 dark:border-purple-500/20 bg-white/80 dark:bg-gray-800/80 text-[10px] resize-none focus:outline-none"
                                    autoFocus
                                  />
                                </div>
                              ) : candidate.notes ? (
                                <div
                                  className="mt-1 flex items-start gap-1 px-1 group/notes cursor-pointer"
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    setEditingNotesId(candidate.id)
                                    setEditingNotesText(candidate.notes || '')
                                  }}
                                  title={candidate.notes}
                                >
                                  <svg className="w-3 h-3 mt-0.5 text-purple-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                  </svg>
                                  <span className="text-[10px] text-gray-400 dark:text-gray-500 line-clamp-1 group-hover/notes:text-purple-500 transition">
                                    {candidate.notes.length > 20 ? candidate.notes.slice(0, 20) + '...' : candidate.notes}
                                  </span>
                                </div>
                              ) : (
                                <button
                                  className="mt-1 px-1 text-[10px] text-gray-300 dark:text-gray-600 hover:text-purple-400 transition"
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    setEditingNotesId(candidate.id)
                                    setEditingNotesText('')
      ...
      {popoverData && (() => {
        ...
        return (
          ...
          >
            ...
            <button
              onClick={() => {
                setEditingNotesId(popoverData.candidateId)
                const cand = workflowCandidates.find(c => c.id === popoverData.candidateId)
                setEditingNotesText(cand?.notes || '')
                setPopoverData(null)
              }}
              className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-500/10 text-xs whitespace-nowrap transition text-purple-500"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
