...
export default function WorkflowPanel() {
  ...
  const handleCreateRun = async () => {
    const name = window.prompt('Workflow run name:', 'New Character')
    if (name) await createWorkflowRun(name)
  }

  const handleDeleteRun = async (runId: string) => {
    if (window.confirm('Delete this workflow run and all its candidates?')) {
  ...
  return (
    <div className="fixed right-0 top-14 bottom-0 w-80 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-l border-gray-200 dark:border-white/[0.08] shadow-2xl z-40 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/[0.06]">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Workflow</h2>
      ...
      <div className="px-3 py-2 border-b border-gray-100 dark:border-white/[0.06]">
        <label className="text-[10px] text-gray-400 uppercase tracking-wider mb-1 block">Active Run</label>
        <select
        ...
        >
          ...
          {workflowRuns.map((run) => (
            <option key={run.id} value={run.id}>{run.name} (Stage {run.currentStage})</option>
          ))}
        </select>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        {!activeRun ? (
          <div className="text-center text-gray-400 text-xs mt-8">
            <p>No active workflow run.</p>
            <p className="mt-1">Create one or select from above.</p>
          </div>
        ) : runCandidates.length === 0 ? (
          <div className="text-center text-gray-400 text-xs mt-8">
            <p>No candidates yet.</p>
            <p className="mt-1">Open a task detail and click</p>
            <p>  Add to workflow  </p>
            <p>to start building your character.</p>
          </div>
        ) : (
